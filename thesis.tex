...
\title{Patient-Specific Parameter Estimation in Mechanistic Insulin-Glucose Models}
...
Any dynamic model has intrinsic approximation errors, and the errors should be taken into account. The approximation error for the states can be modeled as a random process, and artificial process noise can be added into $x_k$ at each time step. This step is important step to accommodate for errors due to unaccounted dynamics, and for notation purposes,the artificial process noise will be omitted from the probabilistic representation in Equation \eqref{eq:prob1}. Choices in process noise will be discussed in Chapter \ref{ch:methodology}.
...
\begin{equation}
    ...
    \end{aligned}
\end{equation}
In the Kalman filter case, such dynamic and measurement models were assumed to be linear and Gaussian, as a result, the estimate of the state $x_k$ was derived as a linear transformation of the mean and covariance. For the bootstrap particle filter, a point estimate of the state from the posterior distribution is formulated as a Markov Chain approximation, with $x_k^{(i)}$ denoting a randomly sampled state for $N$ total particles
...
The steps to implement to formulate the bootstrap particle fiter iteratively can be summarized in the following algorithm \cite{sarkka_bayesian_nodate} \cite{liu_monte_2004}
\begin{enumerate}
...
One method to compensate for singularity is to also add artificial process noise $Q_{k}$ in unknown parameters to allow for random walking in the state space, thus, accounting for errors in the dynamic function
\begin{align}
...
A nonlinear, and nonconvex integral-based parameter identification method has been introduced that can estimate patient-specific parameters through maximum likelihood estimation \cite{hann_integral-based_2005} . By linearly interpolating between the blood glucose measurements in times $[t_0,t_1]$,
\begin{align}
...
\begin{figure}[ht]
    ...
    \caption[Blood glucose dynamic example]{Blood glucose and its' corresponding measurements from the patient model\footnotemark.}
