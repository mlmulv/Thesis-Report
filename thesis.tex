...
\begin{align}
    ...
    \end{bmatrix}.
\end{align}
If it assumed that the patient model starts at steady state, it is possible to derive the initial states with the input and parameter values by solving for the roots of the Differential Equations \eqref{eq:BG} to \eqref{eq:uen}. This can be computed with the solver function \texttt{scipy.optimize.fsolve}\footnote{\url{https://docs.scipy.org/doc/scipy/reference/generated/scipy.optimize.fsolve.html}} for each patient.
...
\section{Observability in Insulin-Glucose Dynamic Models}
...
\begin{equation}
    ...
    \end{aligned}
\end{equation}
as well with a maximum perturbation constant of $c=0.1$. With the following defined inputs, the observability gramian $W_O$ can be derived to analyze the singular values of all the calculated matrices using single value decomposition. 

\section{Nonlinear State Estimation}
The extended Kalman filter and the particle filter are two different nonlinear state estimation methods presented in Section \ref{sec:state}. To compare each estimator with the Intensive Control Insulin-Nutrition-Glucose model, the computational complexity $\mathcal{C}$ of estimating all states over the total time and the mean squared error $\,MSE$ of the blood glucose estimates will be recorded for all patients and filters in the given virtual patient cohort. The simulations will span 12 hours with a discrete time-step of 1 minute, with glucose measurements corrupted with additive Gaussian noise ($N(0,0.25^2)$) sampled every 120 minutes. There will be iterations of this comparison with the following offsets of the initial states: $\pm2\%,\pm10\%,\pm25\%$, with each iteration having a virtual cohort of 500 patients. Adding an offset to the initial state will gauge how each well estimator can adapt to various initial innovations in the estimate. 

There will be artificial process noise added to the state estimates for both the extended Kalman filter and the particle filter. This is to ensure that model approximation errors can be modeled as a random process. Due to the discrete time-step to measurement time-step being a ratio of 120:1, the process noise per sample was chosen as an expected amount of deviation between measurements divided by 120. Due to the discrete time-steps being relatively small and having constant inputs, the process noise $w$ will be sampled from the following random process at each sample
...
\end{align}
The extended Kalman filter parameters are defined as
\begin{enumerate}
...
\end{enumerate}
The number of particles for the particle filter is a hyperparameter that will be analyzed within this section. The particle filters containing $\left(100,\,200,\,300,\,400,\,500,\,750,\,1000,\,1500,\,2000\right)$ particles will be used to compare the relationship of the estimation performance over an increasing number of particles. The particle filter will initialize the particles by sampling from a multivariate Gaussian distribution with a mean of the initial state, and a covariance equal to the extended Kalman filter's $Q$.

To define the computational complexity given the experimental setup, let $\tau_i$ denote the average total time to run each particle filter and the extended Kalman filter in seconds, with $N$ being the total number of filters
\begin{equation}
    \begin{aligned}
        \tau_{\text{min}} &= \min(\tau) \\
...
\Bigg).
\end{align}
The extended Kalman filter is not well equipped to handle log normal dynamics. As a result, the process noise matrix $Q$ has to differ slightly to compensate. The extended Kalman filter configuration for this section is the following
\begin{enumerate}
    ...
    \item Measurement noise of $R = 0.25^2$
\end{enumerate}
The particle filter will contain the "optimal" number of particles chosen from the previous section, and an initial covariance matrix of $\mathrm{diag}\left(1,\,3,\,3,\,1,\,2,\,1,\,3\times10^{-2}\right)$. Assessing the estimation of the insulin sensitivity will be evaluated with the mean absolute relative difference $\,\text{MARD}$ of the true insulin sensitivity $\,S_I$ and the estimated insulin sensitivity $\hat{\,S_I}$ for both predictions from the extended Kalman filter and the particle filter over $K$ patients and $L$ total time-steps 
\begin{equation}
    \begin{aligned}
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.90\linewidth]{figures/5_3.pdf}
    \caption[Empirical Observability Gramian PMF]{Probability mass function of the eigenvalues across all calculated empirical observability gramians}
\end{figure}

\begin{figure}[ht]
   \centering
   \includegraphics[width=0.90\linewidth]{figures/5_6.pdf} 
   \caption[Estimator Comparison]{Comparison of the computational complexity and error across all estimators}
\end{figure}
...
\begin{table}[htbp]
    ...
    \caption{MSE for different estimators and deviations of the initial state}
    \begin{tabular}{l
                S[table-format=1.3]
                S[table-format=1.3]
                S[table-format=1.3]
        ...
        & {-25\%} & {-10\%} & {-2\%} & {2\%} & {10\%} & {25\%} & {} \\
        \midrule
        EKF      & 0.265 & 0.087 & 0.040 & 0.028 & 0.030 & 0.117 & 0.095 \\
        PF 100   & 0.057 & 0.031 & 0.035 & 0.037 & 0.035 & 0.038 & 0.039 \\
        PF 200   & 0.055 & 0.033 & 0.035 & 0.036 & 0.035 & 0.037 & 0.038 \\
        PF 300   & 0.051 & 0.033 & 0.035 & 0.035 & 0.035 & 0.036 & 0.037 \\
        PF 400   & 0.046 & 0.033 & 0.034 & 0.035 & 0.035 & 0.036 & 0.037 \\
        PF 500   & 0.046 & 0.033 & 0.034 & 0.035 & 0.035 & 0.036 & 0.037 \\
        PF 750   & 0.046 & 0.033 & 0.034 & 0.034 & 0.035 & 0.035 & 0.036 \\
        PF 1000  & 0.038 & 0.033 & 0.034 & 0.035 & 0.035 & 0.036 & 0.035 \\
        PF 1500  & 0.036 & 0.034 & 0.034 & 0.035 & 0.036 & 0.036 & 0.035 \\
        PF 2000  & 0.037 & 0.033 & 0.035 & 0.035 & 0.036 & 0.037 & 0.035 \\
        \bottomrule
    \end{tabular}
\end{table}
