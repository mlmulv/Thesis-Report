...
\end{aligned}
\end{equation}
where $p(x_{k} | x_{k-1}, u_{k})$ and $p(y_{k} | x_{k})$ are the stochastic discrete-time dynamic and measurement models, with $x_{k} \in\mathbb{R}^n $ representing the states, $u_{k} \in\mathbb{R}^p$ are the inputs, and $y_{k} \in\mathbb{R}^m$ are the measurements.
...
\end{aligned}
\end{equation}
where $q_{k-1} \sim N(0, Q_{k-1})$ is the process noise, $r_{k} \sim N(0,R_k)$ is the measurement noise, we can rewrite the Equation \eqref{eq:prob1} as the following
\begin{equation}
\begin{aligned}
...
\begin{align}
    ...
    \delta_i &= c \cdot S \cdot t_i, \qquad \text{with } S=\text{diag}(x_{\text{max}}).
\end{align}
...
\begin{align}
x_s = \sqrt{\text{diag}(W_O)} && T_s = \text{diag}^{-1}(x_s).
\end{align}
...
\begin{align}
\theta_k = \theta_{k-1} + \epsilon_{k-1}, && \text{with } \epsilon_{k-1} \sim N(0, Q_{k-1}).
\end{align}
...
\begin{figure}[ht]
    ...
    \caption[Blood glucose dynamic example]{Blood glucose and its' corresponding measurements from an example patient model.}
...
\begin{enumerate}
    ...
    \item \label{RQ: 2} \textbf{How do state estimation techniques compare in estimating patient insulin-glucose dynamics in terms of accuracy and computational complexity?} 
    \item \label{RQ: 3} \textbf{How do practical parameter estimation methods perform relative to the integral-based parameter identification method in terms of estimating unknown patient-specific parameters?}
\end{enumerate}
...
A virtual patient cohort will be simulated to model clinical insulin-glucose data observed in patients in the intensive care unit. A patient model consists of simulated insulin-glucose dynamics ($G, Q, I, \,P1, \,P2, u_{\,en}$, P) represented in Equations \eqref{eq:BG} to \eqref{eq:P}, known insulin sensitivity $S_I(t)$, exogenous insulin $u_{ex}(t)$, enteral feed $D(t)$, and parenteral feed $\,PN(t)$. The population constants listed in Table \ref{tab:params} are assigned to the constant parameters for all virtual patients. To introduce variance between patients, a virtual patient is given a random constant input observed from doses given to patients in the Karolinska University Hospital
\begin{equation}
    \begin{aligned}
...
\begin{align}
    SI &= \exp(X), \qquad \text{ with } X \sim N(-8.05, 0.70^2).
\end{align}
% \begin{figure}[ht]
%     \centering
...
\begin{align}
    x_0 &= \begin{bmatrix}
        G_0 & I_0 & Q_0 & \,P1_0 & \,P2_0 & \, P_0 & \,{u_{en}}_0
    \end{bmatrix}.
\end{align}
...
To answer the given Research Questions \ref{RQ: 1}, \ref{RQ: 2}, and \ref{RQ: 3}, statistical metrics such as Mean Square Error and Mean Absolute Relative Error, and mathematical results (e.g. observability and computational complexity of estimators) will be used to utilized to quantify the results of the experiment such that the hypotheses can be tested.
...
The maximum value of each state\footnote{For this calculation, $u_{en}$ and $P$ are omitted. This is because permutation of these values are directly represented by dynamics $I$ and $P2$.} $x_{max}$ will be chosen as $S$
\begin{equation}
    \begin{aligned}
        S &= \text{diag}(x_{max}) \\
...
The extended Kalman filter and the particle filter are two different nonlinear state estimation methods presented in Section \ref{sec:state}. To compare each estimator with the Intensive Control Insulin-Nutrition-Glucose model, the computational complexity $\mathcal{C}$ of estimating all states over the total time and the mean squared error $\epsilon$ of the blood glucose estimates will be recorded for all patients and filters in the given virtual patient cohort. The simulations will span 12 hours with a time-step of 1 minute, with glucose measurements corrupted with additive Gaussian noise ($N(0,0.25^2)$) sampled every 120 minutes. There will be iterations of this comparison with the following offsets of the initial states: $\pm2\%,\pm10\%,\pm25\%$, with each iteration having a virtual cohort of 500 patients. Adding an offset to the initial state will gauge how each well estimator can adapt to various initial innovations in the estimate. 

There will be artificial process noise added to the state estimates for both the extended Kalman filter and the particle filter. This to ensure that model approximation errors can be modeled as a random process. Due to the discrete time step to measurement time step being a ratio of 120:1, the process noise per sample was chosen as an expected amount of deviation between measurements divided by 120. Due to the discrete timesteps being relatively small and having constant inputs, the following process noise was chosen after fine-tuning
\begin{align} \label{eq:processnoise}
    w &= \text{diag}(5e-6, 1e-3, 1e-3, 5e-7, 2.5e-5, 1e-8).
\end{align}
The extended Kalman filter parameters are defined as
\begin{enumerate}
    \item Initial covariance matrix of $P_0 = \text{diag}(1,3,3,1,2,1)$
    \item Process noise matrix $Q = \text{diag}(5e-6, 1e-3, 1e-3, 5e-7, 2.5e-5, 1e-8)$
    \item Measurement noise $R = 0.25^2$
\end{enumerate}
The number of particles for the particle filter is a hyperparameter that will be analyzed within this section. The particle Particle filters containing $\left(100, 200, 300, 400, 500, 750, 1000, 1500, 2000\right)$ particles will be used to compare the relationship of the outputs over an increasing number of particles. The particle filter will initialize the particles by sampling from a multivariate Gaussian distribution with a mean of the initial state, and a covariance equal to Equation \eqref{eq:processnoise}.
...
\begin{equation}
    ...
    \end{aligned}
\end{equation}
For a true blood glucose sequence $G$ and an estimated blood glucose sequence $\hat{G}$ of length $L$, the mean squared error $\epsilon$ over $K$ total patients is calculated as
\begin{align}
    \epsilon_i &= \frac{1}{L} \sum_{j=1}^{L} \left(G_i[j] - \hat{G}_i[j]\right)^2 & i=1,\dots,K.
\end{align}
...
The Integral-based Parameter Identification method introduced in Section \ref{sec:integral} will be the baseline implementation of estimating insulin-sensitivity $\,SI$ in patients and will be compared against the extended Kalman filter and the particle filter with state augmentation. Each method will try to estimate a constant insulin sensitivity and a piecewise insulin sensitivity that changes abruptly about half-way through the experiment.  
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.9\linewidth]{figures/5_1.pdf}
    \caption{blah} 
\end{figure}

\begin{figure}[ht]
    \centering
    \includegraphics[width=0.9\linewidth]{figures/5_2.pdf}
    \caption{blah blah} 
\end{figure}

\begin{figure}[ht]
    \centering
    \includegraphics[width=\linewidth]{figures/5_3.pdf}
...
%     \centering
%     \begin{threeparttable}
%         \caption{Comparing different estimators based on their respective time complexity and predictive performance regarding blood glucose.}
%         \label{tab:Est_Comp}
%         \begin{tabular}{l || c c c c}
%             \toprule
%             \textbf{Estimator}  & \textbf{Median (Run Time, s)} & \textbf{IQR (Run Time, s)} & \textbf{Median (MSE)} & \textbf{IQR (MSE)} \\
%             \midrule
%             EKF & \textbf{0.3252} & 0.0092 & 0.3998 & 0.0356 \\
%             PF ($N=500$) & 129.1 & 1.686 & 0.3991 & 0.0400 \\
%             PF ($N=750$) & 193.6 & 1.870 & \textbf{0.3954} & 0.0364 \\
...
%         \begin{tablenotes}
%             \footnotesize
%             \item The extended Kalman Filter is denoted as EKF, with PF representing the particle filter with $N$ total particles. 
%         \end{tablenotes}
%     \end{threeparttable}
% \end{table}
