...
When revisiting the probabilistic state estimators in section \ref{sec:state}, to analytically compute the point estimate of the states $\hat{x}$, given all observations at time step $K$, one must evaluate the expected value of the posterior probability
...
\begin{equation}\label{eq:prior2}
    \begin{aligned}
        \hat{x}_k &\approx \frac{1}{N} \sum_{i=1}^{N} g(x^i) p(x^i \mid y_{1:K}).
    \end{aligned}
\end{equation}
...
% \sweExpl{Mindre relaterat arbete 1}

\subsection{Minor related work n}
% \sweExpl{Mindre relaterat arbete n}
