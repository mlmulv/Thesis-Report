...
While the proposed clinically-validated model for patients in the intensive care unit has shown to capture long-term insulin-glucose dynamics, in practice, typically it is only possible for clinicians to take blood glucose measurements, consequently, excluding measurements for the dynamics seen in Equations \eqref{eq:Q} to \eqref{eq:P2}.

\section{Observability and Identifiability}

In the scope of this paper, assessing observability and identifiability must be assessed within the context of nonlinear insulin-glucose dynamical models. Utilizing the observability gramian $W_O$ has traditionally been used in model reduction of linear dynamic systems, but empirical methods have been adapted to accommodate nonlinear systems \cite{hahn_improved_2002}. The observability gramian gives insight into which states and/or parameters have the most influence in the relationship of inputs and outputs of the system by analyzing the rank condition of the matrix.
...
\end{align}
The scalar constant $c$ corresponds to the magnitude of deviation, $S \in \mathcal{R}^{n,n}$ is the diagonal matrix of the maximum of each state and parameter. The row vector $t_i \in R^n$ corresponds to the current perturbation of the orthonormal matrix $T = \begin{bmatrix}
    t_1 & t_2 & \dots & t_{2^n}
...
To make inferences about the observability and identifiability of the observability gramian, one can check the rank condition of the matrix by analyzing the eigenvalues of $W_0$. The eigenvalues close to zero can be classified as not observable. If the states have varying magnitudes of scale, it is recommended \cite{eberle_identifiability_2012} to rerun the observability gramian algorithm again with the following transformations
\begin{align}
    x_s = \sqrt{\text{diag}(W_O)} && T_s = \text{diag}^{-1}(x_s).
\end{align}
The new state perturbations and perturbation matrix are now assigned as the transformation as such $\bar{x} = T_s x$ and $\bar{T} = T_s T$. If the system is found to be weakly observable or not observable after calculating the eigenvalues with the transformed parameters, one can not learn about a subset of the dynamics of the system directly from the measurements. State estimators will be discussed in the following sections, and can account for the lack of intuition into the dynamics not observable through various probabilistic filtering mechanisms.
