...
The Intensive Control Insulin-Nutrition-Glucose model presented by Lin et al. \cite{lin_physiological_2011} describes the insulin-glucose dynamics in critically ill patients. The model is constructed as a fusion of two existing models validated for patients in the intensive care unit \cite{chase_model-based_nodate} \cite{lotz_monte_2008}, with the following ensemble of ordinary differential equations as shown
\begin{align}
...
\end{align}
Equation \ref{eq:BG} is the independent insulin glucose removal, where $BG$ is the total blood glucose concentration (mmol/L). The insulin pharamacodynamics are shown in equations \ref{eq:Q} and \ref{eq:I}, with $Q$ and $I$ being the interstitial insulin concentration (mU/L) and the plasma insulin concentration (mU/L) respectively. The exogenous insulin input is represented as $u_{ex}(t)$ (mU/min), and the endogenous insulin production $u_{en}(t)$ (mU/min) can be measured from C-Peptide, but if measurements are not feasible, the kinetics can modeled as such in equation \ref{eq:uen}. The gastric absorption of glucose is defined in equations \ref{eq:P1} - \ref{eq:P}. The dynamics $P1$ and $P2$ describe the glucose in the stomach and the gut (mmol), with $P(t)$ representing the glucose appearance in the blood (mmol/min). Any enteral food intake given to the patient is accounted for by $D(t)$ (mmol/min), and additional parenteral dextrose is described by $PN(t)$ (mmol/min).

It is of great importance to note that while the proposed model can capture the insulin-glucose dynamics, in practice, typically it is only possible for the clinician to take blood glucose measurements, not for the dynamics in equations \ref{eq:Q}-\ref{eq:P2}. Thus, leveraging estimators is a viable solution to estimate the insulin-glucose dynamics over time to account for the lack of intuition into the dynamics that can not be measured.
...
\begin{equation}
    ...
    \end{aligned}
\end{equation}
The closed-form solution of $p(x_{k} \mid y_{1:k})$ for linear and Gaussian dynamic and measurement models is known as the Kalman filter, with two key parts: the \textit{prediction step} and the \textit{update step}.
\begin{enumerate}
    ...
    \end{equation}
\end{enumerate} 
The Kalman Filter is a recursive, as the filter starts with an initial point estimate of the mean $m_0^-$ and the covariance $P_0^-$, and iterates over discrete time steps $k$. While the Kalman filter is a great starting point for an estimator, as the assumptions of linearity and Gaussianity often hold in many contexts, it is clear when analyzing the equations \ref{eq:BG}-\ref{eq:P2}, the ordinary differential equations are not linear, consequently, the estimators of choice must have the capability to properly handle nonlinearities in the state equations.

\subsection{Extended Kalman filter}

The extended Kalman filter expands upon the linearity condition to allow for nonlinear models. The structure of the extended Kalman filter remains the same as equations \ref{eq:Predict} and \ref{eq:Update}.
