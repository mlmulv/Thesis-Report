...
\item Formulate and evaluate estimators for chosen patient-specific parameters 
...
Chapter \ref{ch:background} will introduce the existing modeling of insulin-glucose dynamics used, followed by an overview of state estimation and parameter estimation techniques. In Chapter \ref{ch:methods}, the research method will be motivated along with the data collection. The research methodology will be presented in Chapter \ref{ch:whatYouDid} such that the steps to answer the research questions are clearly documented. The results will be shown in Chapter \ref{ch:resultsAndAnalysis}, and the analysis of the results will proceed in Chapter \ref{ch:resultsAndAnalysis}. Finally, the overall conclusion, key findings, and future work of the experiment will be summarized in Chapter \ref{ch:conclusionsAndFutureWork}.
...
\begin{alignat}{1}
 ...
 P(t) &= min(d_{2}P2,P_{max}) + P_N(t) \label{eq:P}
\end{alignat}
...
While the proposed clinically-validated model for patients in the intensive care unit has shown to capture long-term insulin-glucose dynamics, in practice, typically it is only possible for clinicians to take blood glucose measurements, consequently, excluding measurements for the dynamics seen in Equations \eqref{eq:Q} to \eqref{eq:uen}.

\section{Observability and Identifiability} \label{sec: obs}
...
\end{aligned}
\end{equation}
where $p(x_{k} | x_{k-1}, u_{k})$ and $p(y_{k} | x_{k})$ are the discrete-time dynamic and measurement models, with $x_{k} \in\mathbb{R}^n $ representing the states, $u_{k} \in\mathbb{R}^p$ are the inputs, and $y_{k} \in\mathbb{R}^m$ are the measurements. The objective is to ultimately derive the distribution of the state $x_k$ , given all the measurements up to the current sample $p(x_{k} \mid y_{1:k})$.
...
\end{aligned}
\end{equation}
The closed-form solution of $p(x_{k} \mid y_{1:k})$ for linear and Gaussian dynamic and measurement functions is known as the Kalman filter, with a full derivation available from Sräkkä and Svensson \cite{sarkka_bayesian_nodate}. The filter contains two key parts: the \textit{prediction step} and the \textit{update step}.
\begin{enumerate}
...
\footnotetext{The initial conditions of the patient model are $BG = 5.0$ (mmol/L), $I = 15.0$ (mU/L), $Q = 15.0$ (mU/L), $P1 = P2 = 0.0$ (mmol). The inputs are $u_{ex}(t) = 75 \cdot e^{-\left(\frac{\log(2)}{300}\right) \cdot \left((t + 120) \mod (300)\right)}$ (mU/min), $P_N(t) = e^{-\left(\frac{\log(2)}{300}\right) \cdot \left(t \mod (300)\right)}$ (mmol/min),
and no external nutrition is given. Measurements are sampled with additive Gaussian noise $r_k \sim N(0,0.25^2)$.}
...
\begin{figure}[ht]
    ...
    \caption{System Inputs and Parameters Probability Density Function}
\end{figure}
...
\begin{figure}[ht] 
    ...
    \caption{Initial States Probability Density Function}
...
A singular patient model was randomly selected to investigate the observability of the Intensive Control Insulin-Nutrition-Glucose model with the following nominal state vector\footnote{While $u_{en}$ is a process variable, the numerical integration method solves for the Differential Equations $\eqref{eq:BG}$ to $\eqref{eq:P2}$. Thus, permutations in $u_{en}$ do not result in a change in $G$. For the observability gramian calculation, $u_{en}$ is omitted from $x_0$.} $x_0 \in \mathbb{R}^5$
 \begin{equation}
    \begin{aligned}
        x_0 &= \begin{bmatrix}
            10.80 & 47.03 & 27.64 & 10.26 & 51.31
        \end{bmatrix} \\
        u_{ex}(t) &= 24.21 \\
        D(t) &= 0.23 \\
        \,PN(t) &= 0.36 \\
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.9\linewidth]{figures/5_1.png}
    \caption[Analysis of the integral-based parameter identification method]{The performance of approximating insulin sensitivity with the integral-based method across various measurement noise.}
    \label{fig:Int_Error}
\end{figure}
