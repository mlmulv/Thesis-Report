...
\begin{align}\label{eq:gramian}
    ...
    y_k &= h(x_k).
\end{align}
To empirically calculate the observability gramian, we first generate $2^n$ unique combinations of the initial state $x_0 \in \mathcal{R}^n$, with $n$ denoting the numbers of states and parameters.
The initial condition $x_0$ will be assigned as perturbations $\delta_i \in \mathcal{R}^n$ around a nominal value $x_{\,nom} \in \mathcal{R}^n$
\begin{align}
    x_{0,i} &= x_{\,nom} + \delta_i \nonumber \\
    \delta_i &= c \cdot S \cdot t_i && \text{with } S=\text{diag}(x_{\text{max}}).
\end{align}
The scalar constant $c$ corresponds to the magnitude of deviation, $S \in \mathcal{R}^{n,n}$ is the diagonal matrix of the maximum of each state and parameter. The row vector $t_i \in R^n$ is the specified perturbation $i$ from the orthonormal matrix $T = \begin{bmatrix}
    t_1 & t_2 & \dots & t_{2^n}
...
\begin{align}
    \varphi_k(\theta) = \varphi_{k-1}(\theta) + \frac{1}{2} \log \left|2 \pi S_k(\theta)\right| + \frac{1}{2} \log v_k^T(\theta)S_k(\theta)v_k(\theta).
\end{align}
...
\begin{itemize}
    \item $k=0$
    \begin{enumerate}
        \item assign an initial candidate $\theta_{0}$ and the initial energy function value $\varphi_0(\theta_0) = - \log p(\theta$)
    \end{enumerate}
    \item for $k=1:K$
        \begin{enumerate}
...
A virtual patient cohort will be simulated to model clinical insulin-glucose data observed in patients in the intensive care unit. A patient model consists of simulated insulin-glucose dynamics ($G, Q, I, P1, P2$) represented in Equations \eqref{eq:BG} to \eqref{eq:P2}, known insulin sensitivity $S_I(t)$, exogenous insulin $u_{ex}(t)$, and enternal feed $D(t)$. The population constants listed in Table \ref{tab:params} are assigned to the constant parameters for all virtual patients. To simulate patient data, each virtual patient (100 total patients) is given an initial state for the dynamics $x_0$.
Blood glucose concentration (mmol/L) has been demonstrated to have an underlying log-normal distribution \cite{davidson_multi-input_2019}, thus, for the purpose of this experiment, we can define blood glucose as the following random variable $\ln G \sim N(2.02, 1.3^2)$, with each patient being assigned an unique initial state of
\begin{align}
    x_0 &= \begin{bmatrix}
        G_0 & Q_0 & I_0 & \,P1_0 & \,P2_0
    \end{bmatrix} \nonumber \\
    &= \begin{bmatrix}
        \exp(\ln G) & 15.0 & 15.0 & 0.0 & 0.0
    \end{bmatrix}.
\end{align}
$Q_0$ and $I_0$ are both chosen as $15.0$ (mU/L) as the initial interstitial and plasma insulin concentration are in line with measurements collected to model an insulin-glucose system \cite{dalla_man_meal_2007}. The initial glucose concentration (mmol/L) in the stomach $\,P1_0$ and gut $\,P2_0$ are assumed to be zero at the beginning of insulin infusion and feeding. All patients are given continuous ($t=$ mins) exogenous insulin input
\begin{align}
    u_{ex}(t) = 140 \exp \left(-\frac{\log 2}{300} \left(t \mod{300}\right)\right) \text{ (mU/min)},
\end{align} and a constant enternal intake of glucose
\begin{align}
    D(t) = 0.30 \text{ (mmol/min)}.
\end{align}
The units of insulin per hour (U/hr) can be derived as
\begin{align}
    \frac{1}{1000} \int_{t_1}^{t_1+60}u_{ex}(t) dt \approx 7.84 \text{ (U/hr)} && \text{for } t > 300,
\end{align}
as well as the total caloric intake of glucose per day 
\begin{align}
    \frac{0.30 \text{ (mol/min)}}{1000} \times 180.156 \text{ (g/mol)} \times 1440 \text{ (min)} \times 4 \text{(kcal/day)} \approx 304 \text{ (kcal/day)},
\end{align} with both aligning in typical ranges given to ICU patients in the Karolinska University Hospital.

Given all the initial states $x_0$, known insulin sensitivity $S_I(t)$, and the patient inputs $u_{ex}(t), D(t)$, the patient cohort can be generated by solving the Differential Equations \eqref{eq:BG} to \eqref{eq:P2} from times 0 mins to 3600 mins with a time step of $dt = 1$ min for each patient. The ODE solver \texttt{scipy.integrate.solve\_ivp}\footnote{\url{https://docs.scipy.org/doc/scipy/reference/generated/scipy.integrate.solve\_ivp.html}} function in python will be applied with the Explicit Runge-Kutta of order 5(4) integration method \cite{dormand_family_1980} to solve for the necessary dynamics.  


\section{Assessing reliability and validity of the data collected}
\label{sec:assessingReliability}

A virtual patient cohort is a simplified model to characterize the target population of patients in the intensive care unit, given certain assumptions of the patients. Each patient will have their unique insulin-glucose model that contains all the recorded dynamics and inputs in time.

\subsection{Validity of method}
\label{sec:validtyOfMethod}

The patient models, parameters, and initial states are all adopted and cross-validated from multiple clinically validated studies. This provides a physiologically accurate basis to the control variables of the experiment. Interventions in the experiment can be assessed and compared knowing the manipulated variable is correlated with the change in the output and not due to poor model formulation. 
...
\subsubsection{Integral-Based Fitting}
