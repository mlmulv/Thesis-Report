...
The Intensive Control Insulin-Nutrition-Glucose model presented by Lin et al. \cite{lin_physiological_2011} characterizes the insulin-glucose dynamics in critically ill patients, and incorporates additional methods beyond solely using insulin infusion for gluco-regulation. The model is constructed as a fusion of two existing models validated for patients in the intensive care unit \cite{chase_model-based_nodate} \cite{lotz_monte_2008}, with the following ensemble of ordinary differential equations shown as
\begin{align}
 ...
 \dot{I} &= -n_{K}I(t) - n_{L}\frac{I(t)}{1+\alpha_{I}I(t)} - n_{I}(I(t)-Q(t)) + \frac{u_{ex}(t)+(1-x_{L})u_{en}(t)}{V_I} \label{eq:I} \\
...
\end{align}
Equation \ref{eq:BG} is the independent insulin glucose removal, where $BG$ is the total blood glucose concentration (mmol/L). The insulin sensitivity $S_I$ is time varying and crucially gives insight to the long-term evolution of the patient's health over time. The insulin pharamacodynamics are shown in equations \ref{eq:Q} and \ref{eq:I}, with $Q$ and $I$ being the interstitial insulin concentration (mU/L) and the plasma insulin concentration (mU/L) respectively. The exogenous insulin input is represented as $u_{ex}(t)$ (mU/min), and the endogenous insulin production $u_{en}(t)$ (mU/min) can be measured from C-Peptide, but if measurements are not feasible, the kinetics can modeled as such in equation \ref{eq:uen}. The gastric absorption of glucose is defined in equations \ref{eq:P1} - \ref{eq:P}. The dynamics $P1$ and $P2$ describe the glucose in the stomach and the gut (mmol), with $P(t)$ representing the glucose appearance in the blood (mmol/min). Any enteral food intake given to the patient is accounted for by $D(t)$ (mmol/min), and additional parenteral dextrose is described by $PN(t)$ (mmol/min).

It is of great importance to note that while the proposed model can capture the insulin-glucose dynamics, in practice, typically it is only possible for clinicians to take blood glucose measurements, consequently, excluding measurements for the dynamics seen in equations \ref{eq:Q}-\ref{eq:P2}. Thus, leveraging estimators is a viable solution to estimate the insulin-glucose dynamics over time to account for the lack of intuition into the dynamics that can not be measured.
...
\begin{equation} \label{eq:prob1}
    ...
    \end{aligned}
\end{equation}
where $p(x_{k}, u_{k} | x_{k-1})$ and $p(y_{k} | x_{k})$ are the discrete-time dynamic and measurement models, $x_{k} \in\mathbb{R}^n $ are the states, $u_{k} \in\mathbb{R}^p$ are the inputs, and $y_{k} \in\mathbb{R}^m$ are the measurements. The objective is to ultimately derive the point estimate $\hat{x_k}$ of the state, given all the measurements up to current sample $p(x_{k} \mid y_{1:k})$.
...
\begin{equation}
    ...
    \end{aligned}
\end{equation}
where $q_{k-1} \sim N(0, Q_{k-1})$ is the process noise, $r_{k} \sim N(0,R_k)$ is the measurement noise, we can rewrite equation \ref{eq:prob1} as the following
\begin{equation}
    \begin{aligned}
    ...
    \end{aligned}
\end{equation}
The closed-form solution of $p(x_{k} \mid y_{1:k})$ for linear and Gaussian dynamic and measurement models is known as the Kalman filter \cite{sarkka_bayesian_nodate}, with two key parts: the \textit{prediction step} and the \textit{update step}.
\begin{enumerate}
...
When revisiting the probabilistic state estimators in section \ref{sec:state}, to analytically compute the point estimate of the states $\hat{x}$, given all observations up to time step $K$, one must evaluate the expected value of the posterior probability
\begin{align}\label{eq:prior}
   \hat{x} = E[g(x) \mid y_{1:K}] = \sum_{x} g(x)p(x \mid y_{1:K}),
\end{align}
where $g(\cdot)$ is an arbitrary distribution $\mathbb{R}^n \rightarrow \mathbb{R}^m$. Such an integral is often difficult to compute in closed-form due to the dimensionality of $x$, but by using drawing $N$ many random samples from the target distribution $x^i \sim g(x)$, the approximation of the point estimate of the states will converge to the true point estimate by the Law of Large Numbers and the Central Limit Theorem \cite{liu_monte_2004}, and can be rewritten as
\begin{equation}\label{eq:prior2}
    \begin{aligned}
        \hat{x} &\approx \frac{1}{N} \sum_{i=1}^{N} g(x^i) p(x^i \mid y_{1:K}).
    \end{aligned}
\end{equation}
The distribution $p(x^i \mid y_{1:K})$ is often unknown, and can not be evaluated, but according to Bayes' rule, it can be rewritten as 
\begin{equation}
    \begin{aligned}
    ...
    \end{aligned}
\end{equation}
where $\pi(x^i \mid y_{1:K})$ is defined as the importance mass, which approximates the posterior by drawing particles from a nonuniform distribution. Sampling from a nonuniform distribution will result in each particle having a corresponding weight $w^{*(i)}$, and a normalized weight $w^i$
\begin{equation}
    \begin{aligned}
...
The bootstrap particle filter is formulated on the principle of Markov Chain Monte Carlo approximations, where the filtering distributions are shown in equation \ref{eq:prob1}. The bootstrap particle filter is ran sequentially like the Kalman filter, and the importance mass is assigned as the dynamic model $p(x_k \mid x_{k-1})$, which results in the following algorithm \cite{sarkka_bayesian_nodate} \cite{liu_monte_2004}

\begin{enumerate}
    \item Sample $N$ particles from the dynamic function
    \begin{align}
        x_k^i = p(x_k \mid x_{k-1}^i) \quad i = 1,\dots, N
    \end{align}
...
\begin{table}
    ...
    \begin{threeparttable}
        ...
        \begin{tabular}{ccc}
            \toprule
            % \multicolumn{2}{c}{Constant Intensive Control Insulin-Nutrition-Glucose Model
            % Parameters} \\
            % \midrule
            ...
            Cellular insulin clearance rate from interstitium & $n_C$ ($\text{min}^{-1}$) & $= n_I$ \\ 
            ...
            Fraction of first pass hepatic extraction & $x_L$ & 0.67 \\
            \multirow{2}{*}{Transport rates between the stomach and the gut} & $d_1$ ($\text{min}^{-1}$) & 0.0347 \\
            & $d_2$ ($\text{min}^{-1}$) & 0.0069 \\
            Maximal out flux from the gut & $P_{max}$ (mmol/min) & 6.11 \\
        ...
        \begin{tablenotes}
            \footnotesize
            \item Parameters shaded in grey are derived from integral fitting, while the rest are assigned as population constants.
        \end{tablenotes}
    \end{threeparttable}
\end{table}
